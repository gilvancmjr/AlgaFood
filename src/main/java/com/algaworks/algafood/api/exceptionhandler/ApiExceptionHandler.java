...
public class ApiExceptionHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
		...
		List<Problem.Field> problemField = bindingResult.getFieldErrors().stream().map(fieldError -> {

			// LocaleContextHolder.getLocale() -> Internacionalização da mensagem de error
			String message = messageSource.getMessage(fieldError, LocaleContextHolder.getLocale());
			return Problem.Field.builder().name(fieldError.getField()).userMessage(message).build();
		}).collect(Collectors.toList());

		Problem problem = createProblemBuilder(status, problemType, detail).userMessage(detail).fields(problemField)
				.build();
